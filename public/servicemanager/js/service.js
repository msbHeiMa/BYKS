        ...
        columns: [
           ...
           },
           {
            ...
            },
            {
                ...
                align: 'center',
                valign: 'middle',
                //events: ISMONITOREvents,
                formatter: ISMONITORFormatter
            },
            {
                title: '操作',
                field: 'operate',
            ...
            $('.i-checkbox').bootstrapSwitch({
                ...
                onSwitchChange: function (event, state) {
                    ...
                    if (name == "ismonitor") {
                        ...
                        jsonresult[0]['ISMONITOR'] = curState;
                        //           var ISMONITOR = document.getElementById("table").rows[id].innerText;
                        $.ajax({
                            type: "PUT",
                            url: restconfig.resturl + "/rest/view/service/" + getQueryString("id") + "/" + data,
                            dataType: 'JSON',
                        ...
                        jsonresult[0]['ISRIGHTMANAGE'] = curState;
                        //       alert(data+"-权限-"+state);
                        $.ajax({
                            type: "PUT",
                            url: restconfig.resturl + "/rest/view/service/" + getQueryString("id") + "/" + data,
...
function ISMONITORFormatter(value, row, index) {
    var checked = row.ISMONITOR == "1" ? "checked" : "";
    return [
        "<div class='switch'><input class='i-checkbox' data='" + row.SERVICEID + "' name='ismonitor' type='checkbox' " + checked + " /></div>"
    ].join('');
}
/*window.ISMONITOREvents = {
 'click .switch': function (value, row, index) {
...
function ISRIGHTMANAGEFormatter(value, row, index) {
    ...
    var checked = row.ISRIGHTMANAGE == "1" ? "checked" : "";
    return [
        "<div class='switch'><input class='i-checkbox' data='" + row.SERVICEID + "' name='isrightmanage' type='checkbox' " + checked + " /></div>"

    ].join('');


}
/* window.ISRIGHTMANAGEEvents = {
