...
function initTable(url, Func) {
    $.ajax({
        ...
        type: "GET",
        data: "",
        url:  zhzlconfig.backendurl + "/zszh/listPeopleDetailById?JSBId=" +getQueryString("JSBId"),
        dataType: 'json',
        contentType: "application/x-www-form-urlencoded; charset=utf-8",
         success: function (result) {
            ...
            case "04": $("#treatS").val("未接受过治疗");
             ...
             $("#birthDate").val(result.data.rows[0].birthDate);
             $("#nation").val(result.data.rows[0].nation);
             $("#nativePlace").val(result.data.rows[0].nativePlace);
             $("#phone").val(result.data.rows[0].phone);
             $("#rAddr").val(result.data.rows[0].rAddr);
             $("#cTroubleCount").val(result.data.rows[0].cTroubleCount);
             $("#preVisitDate").val(result.data.rows[0].preVisitDate);
             $("#dataSources").val(result.data.rows[0].dataSources);
             $("#dangerRank").val(result.data.rows[0].dangerRank);
            var url=zhzlconfig.backendurl +"/zszh/readImageByPsmId?id="+result.data.rows[0].personId;
