...
function initTable() {
    $.ajax({
        ...
        type: "GET",
        data: "",
        url:  zhzlconfig.backendurl + "/zszh/listPeopleById?JSBId=" +getQueryString("zId"),
        dataType: 'json',
...
function save() {
    ...
    var visitPeople=$("#visitPeople").val();
    var visitCom=$("#visitCom").val();
    var remarks=$("#remarks").val();
    var postJson = { "zId": zId, "visitDate": visitDate, "visitPeople": visitPeople, "visitCom": visitCom, "remarks": remarks };
    $.ajax({
        type: "POST",
        url: zhzlconfig.backendurl + "/zszh/addRevisit",
        dataType: 'json',
        data: postJson,
        ...
        success: function (result) {
            if (result.success) {
                alert('保存成功');    
            }window.location.href = "record.html?zId=" + row.pId + "&title=" + row.name + "回访记录 '";
        },
        error: function () {
            alert(' 保存失败');
