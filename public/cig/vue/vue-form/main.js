define([
  ...
  var renders = {
    ...
    groupRender:function(groups, subRender, fields, createElement){
      ...
      return groups.map(function(group){
        return createElement('div', {
          ...
          subRender(fields.filter(function(field){
   ...
   *          {name:"name",label:"姓名",colSpan:"跨列""}//colSpan是布局方法用的属性，如果不是table布局，可以不要这个
   *          支持一些基本字段，类型如下：
    ...
    methods: {
      ...
      getElementByType: function (field, createElement) {
        ...
        switch (type) {
            ...
            return options.map(function (option) {
              return createElement('label', {
                class: {
                  ...
                  renderInput('<input type="radio" v-model="' + (dataPath ? dataPath + '.' : '') + name + '" value="' + option.value + '">', createElement),
                  option.text
                ])
            }, this)
          case 'checkbox:multi':
            ...
            return options.map(function (option) {
              return createElement('label', {
                class: {
                  ...
                  renderInput('<input type="checkbox" v-model="' + (dataPath ? dataPath + '.' : '') + name + '" value="' + option.value + '">', createElement),
                  option.text
                ])
            }, this)
          case 'checkbox':
            ...
            return [
              createElement('label', {
                class: {
                  ...
                  renderInput('<input type="checkbox" true-value="' + trueValue + '" false-value="' + falseValue + '" v-model="' + (dataPath ? dataPath + '.' : '') + name + '">', createElement),
                  createElement("span",{
                      style:{
                        visibility:"hidden"
                      }
          ...
          case 'textarea':
            return [
              renderInput('<textarea class="form-control" rows="3" v-model="' + (dataPath ? dataPath + '.' : '') + name + '"></textarea>', createElement)
            ]
          case 'date':
            return [
              createElement("div",{
                class:{
                  ...
                  renderInput('<input class="form-control" readonly type="text" v-date="1" v-model="' + (dataPath ? dataPath + '.' : '') + name + '">',createElement),
                  createElement("div",{
                      class:{
          ...
          case 'number':
            return [
              renderInput('<input class="form-control" type="number" v-model.number="' + (dataPath ? dataPath + '.' : '') + name + '">', createElement)
            ]
            break
          case 'text':
          default:
            return [
              renderInput('<input class="form-control" type="text" v-model.trim="' + (dataPath ? dataPath + '.' : '') + name + '">', createElement)
            ]
        }
  ...
  return {
    ...
    helper: {
       ...
       * 返回fields 和 默认的data
       */
      getFieldsMixin: function (fields, def) {
        var res = {}
        ...
        return {
          ...
          methods:{
            ...
            resetFormData:function(res){
                this.$set(this,"data",$.extend({},this.getFormOriginData()));
            }
          }
        }
      },
